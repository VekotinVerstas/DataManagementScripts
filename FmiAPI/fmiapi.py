...
from fvhdms import dataframe_into_influxdb, get_default_argumentparser, parse_args, parse_times, save_df, user_agent
...
STATIONS_URL = (
    ...
    "&storedquery_id=fmi::ef::stations&networkid="
)

"""
# Example request URL
https://opendata.fmi.fi/wfs?request=getFeature&
storedquery_id=urban::observations::airquality::hourly::multipointcoverage&geoId=-106948

# List available weather stations

python fmiapi.py -st 20200510T08:00:00Z -et 20200510T20:00:00Z \
       --storedquery fmi::observations::weather::multipointcoverage -i fmisid --stationids 100971 101004 --list hels
...
python fmiapi.py -st 20200510T08:00:00Z -et 20200510T20:00:00Z --storedquery \
    fmi::observations::weather::multipointcoverage -i fmisid --stationids 100971 101004 --list hels --stationtype 151
...
storedquery_id=fmi::observations::weather::multipointcoverage&fmisid=100971&timestep=10

http://opendata.fmi.fi/wfs?request=GetFeature&
storedquery_id=fmi::observations::weather::multipointcoverage&geoid=-16000150&timestep=10
"""
...
def parse_fmi_args() -> argparse.Namespace:
    ...
    parser.add_argument("--timestep", type=int, dest="timestep", default=60, help="timestep parameter value in FMI URL")
    parser.add_argument("--wait", type=float, default=1, help="Time to wait (in seconds) between requests")
    parser.add_argument(
        "--storedquery",
        help="Stored query. Must be multipointcoverage type",
    ...
    parser.add_argument(
        "-i", "--idfield", required=True, default="geoid", choices=["geoid", "fmisid"], help="Id parameter name"
    )
    parser.add_argument(
        "--extraparams",
        nargs="+",
    ...
    parser.add_argument("--list", nargs="?", const="", help="List available stations with optional regex")
    parser.add_argument(
        "--stationtype", default="121", choices=["121", "151"], help="Weather station (121) or airquality station (151"
    )
...
def get_fmi_api_url(
    args: dict, geoid: str, storedquery: str, starttime: datetime.datetime, endtime: datetime.datetime
) -> str:
...
def get_fmi_data(
    ...
    while temp_starttime <= endtime:
        ...
        try:
            (name, lat, lon, t_timestamp_lines, t_data_lines) = get_fmi_data_week_max(
                args, geoid, storedquery, temp_starttime, temp_endtime
            )
    ...
    for i in range(len(timestamp_lines)):
        ...
        parsed_line = {"time": timestmap.isoformat() + "Z", "data": data}
...
def list_fmi_stations(args: dict):
    ...
    if search_re is None:
        ...
        print(
            df[
                (df["name"].str.contains(search_re, flags=re.IGNORECASE))
...
{
 "@gml:id": "WFS-ogZLiFFU.2WTncpsuW8XkiFgHiCJTowsWbbpdOsuZ0659MPTTv3c4W9h...dzAA--",
 "gml:identifier": {
